...
def drawPoints(canvas, instances, color, shape):
    ...
    minX = float(canvas.data["minX"])
    minY = float(canvas.data["minY"])
    maxX = float(canvas.data["maxX"])
    maxY = float(canvas.data["maxY"])
...
dataset = loadCSV("/home/ak/flaskr/csv_kmeans.csv")
